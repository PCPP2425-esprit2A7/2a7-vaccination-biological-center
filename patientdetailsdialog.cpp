    ...
    patientId(id)
{
    ...
    connect(ui->viewButton, &QPushButton::clicked, this, &PatientDetailsDialog::viewDocument);
    connect(ui->saveButton, &QPushButton::clicked, this, &PatientDetailsDialog::saveDocument);
    connect(ui->pdfButton, &QPushButton::clicked, this, &PatientDetailsDialog::exportPDF);
    connect(ui->deleteButton, &QPushButton::clicked, this, &PatientDetailsDialog::deletePatient);
    connect(ui->editButton, &QPushButton::clicked, this, &PatientDetailsDialog::enableEditing);
    connect(ui->modifyButton, &QPushButton::clicked, this, &PatientDetailsDialog::modifyPatient);
    connect(ui->cancelButton, &QPushButton::clicked, this, &PatientDetailsDialog::cancelEditing);
    connect(ui->browseButton, &QPushButton::clicked, this, &PatientDetailsDialog::browseDocument);
...
void PatientDetailsDialog::setFieldsReadOnly(bool readOnly)
{
    ...
    ui->browseButton->setEnabled(!readOnly);
}

void PatientDetailsDialog::enableEditing()
{
    editMode = true;
...
void PatientDetailsDialog::cancelEditing()
{
    if (editMode) {
        loadPatientData(patientId);
...
void PatientDetailsDialog::viewDocument()
{
    if (documentData.isEmpty()) {
    ...
    if (tempFile.open()) {
        tempFile.write(documentData);
        tempFile.close();
        QDesktopServices::openUrl(QUrl::fromLocalFile(tempFile.fileName()));
    } else {
        QMessageBox::warning(this, "Erreur", "Impossible de créer un fichier temporaire.");
    }
}

void PatientDetailsDialog::saveDocument()
{
    if (documentData.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Aucun document disponible.");
        return;
...
void PatientDetailsDialog::exportPDF()
{
...
void PatientDetailsDialog::deletePatient()
{
    ...
    if (reply == QMessageBox::Yes) {
        Crud crud;
        bool success = crud.delete_patient_by_id(patientId, parentTable);
...
void PatientDetailsDialog::browseDocument()
{
...
void PatientDetailsDialog::modifyPatient()
{
    if (!editMode) return;
    ...
    Crud crud;
    crud.update_patient(id.toInt(), nom, prenom, genre, date, fileName, tel.toInt(),
                        fileData.isEmpty() ? documentData : fileData);

    crud.load_patients(parentTable);
