...
Crud::Crud(int IDR, QString TITRER, QString CLASSR, QString DESCR, QString DATEM) {
    this->IDR = IDR;
    this->TITRER = TITRER;
    this->CLASSR = CLASSR;
    this->DESCR = DESCR;
...
void Crud::add_recherche(QTableWidget *tableWidget) {
    ...
    if (db.open()) {
        QSqlQuery thisdb(db);
        thisdb.prepare("INSERT INTO RECHERCHE (IDR, TITRER, CLASSR, DESCR, DATEDR, DATEM) "
                       "VALUES (:IDR, :TITRER, :CLASSR, :DESCR, TO_DATE(:DATEM, 'YYYY-MM-DD'))");
        thisdb.bindValue(":IDR", IDR);
        thisdb.bindValue(":TITRER", TITRER);
        thisdb.bindValue(":CLASSR", CLASSR);
        thisdb.bindValue(":DESCR", DESCR);
        thisdb.bindValue(":DATEM", DATEM);
        ...
        if (thisdb.exec()) {
            ...
            tableWidget->setItem(row, 5, new QTableWidgetItem(DATEM));
