...
void MainWindow::on_ajouter_clicked()
{
    ...
    QString CLASSR = ui->Classer->currentText(); // Get selected value from combobox
    QString DESCR = ui->desc->toPlainText(); // Get text from QTextEdit
    QString DATEDR = ui->dateD->date().toString("yyyy-MM-dd"); // Get date from QDateEdit
    QString DATEFR = ui->dateF->date().toString("yyyy-MM-dd"); // Get date from QDateEdit
    ...
    ui->ID->clear();
    ui->titre->clear();
    ui->Classer->setCurrentIndex(0); // Reset combobox
    ui->desc->clear();
    ui->dateD->setDate(QDate::currentDate()); // Reset to today's date
